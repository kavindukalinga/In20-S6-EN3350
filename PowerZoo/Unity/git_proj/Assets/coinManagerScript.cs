...
public class coinManagerScript : MonoBehaviour
{
    ...
    public IEnumerator calculate_offline_coins() {
        ...
        Debug.Log("Last logging: " + last_logging.ToString());
    ...
    public void calculate_stall_coins(float coins_per_hour) {
        ...
        DateTime last_logging = APIHubScript.Instance.lastLogging.oldTime;
        StartCoroutine(APIHubScript.Instance.put_last_logging());
        DateTime current_logging = APIHubScript.Instance.lastLogging.newTime;
        TimeSpan diff = current_logging - last_logging;
